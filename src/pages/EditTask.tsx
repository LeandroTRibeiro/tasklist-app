        ...
        <>
            ...
            {!loading.status &&
                <div className={`flex justify-center pt-10 pb-10 ${theme.status ? 'bg-white' : 'bg-gray-800'}`}>
                    <form method="POST" onSubmit={handleSubmit} className={`border-2 rounded-lg py-5 px-10 shadow-xl flex flex-col gap-5 ${theme.status ? '' : 'bg-gray-700 border-gray-800'}`}>
                        ...
                        <button className={`btn btn-primary ${theme.status ? '' : 'text-black'}`} disabled={disable}>Salvar</button>
